...
bool ErdHook::FindNeededSignatures() {
	...
	Signature setEvent = Signature("?? ?? ?? ?? ?? 48 89 74 24 18 57 48 83 EC 30 48 8B DA 41 0F B6 F8 8B 12 48 8B "
		"F1 85 D2 0F 84 ?? ?? ?? ?? 45 84 C0");
	...
	*(void**)&EventHook::IsEventFlag = isEvent.Scan(&EldenRingData, Align16);
	...
	Signature disableMap = Signature("74 ?? C7 45 38 58 02 00 00 C7 45 3C 02 00 00 00 C7 45 40 01 00 00 00 48 ?? "
		"?? ?? ?? ?? ?? 48 89 45 48 48 8D 4D 38 E8 ?? ?? ?? ?? E9");
	...
	Signature disableCrafting = Signature("48 83 EC ?? 48 8B 0D ?? ?? ?? ?? 48 85 C9 75 ?? 48 8D 0D ?? ?? ?? ?? E8 "
		"?? ?? ?? ?? 4C 8B C8 4C 8D 05 ?? ?? ?? ?? BA B4 00 00 00 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8B 0D ?? ?? "
		"?? ?? E8 ?? ?? ?? ?? 84 C0 0F 94 C0 48 83 C4 ?? C3", 0x3E);
	...
	Signature findEquipParamWeaponEntrySig = Signature("40 57 41 56 41 57 48 83 EC 40 48 C7 44 24 20 FE FF FF FF 48 "
		"89 5C 24 60 48 89 6C 24 68 48 89 74 24 70 8B");
	ParamMan->FindEquipParamWeaponFunc = (FindEquipParamWeaponEntry*)findEquipParamWeaponEntrySig.Scan(&EldenRingData, Align16);

	Signature findEquipParamProtectorEntrySig = Signature("41 54 41 56 41 57 48 83 EC 40 48 C7 44 24 20 FE FF FF FF "
		"48 89 5C 24 60 48 89 6C 24 68 48 89 74 24 70 48 89 7C 24 78 8B EA 4C 8B F1 33");
	ParamMan->FindEquipParamProtectorFunc = (FindEquipParamProtectorEntry*)findEquipParamProtectorEntrySig.Scan(&EldenRingData, Align16);

	Signature findEquipParamGoodsEntrySig = Signature("45 33 C0 41 8D 50 03 E8 ?? ?? ?? ?? 48 85 C0 0F 84 ?? ?? ?? "
		"?? 48 8B 80 80 00 00 00 48 8B 90", -0x67);
	...
	Signature getMenuCommonParamEntrySig = Signature("40 57 48 83 EC 40 48 C7 44 24 20 FE FF FF FF 48 89 5C 24 50 "
		"48 8B F9 33 DB 48 89 19 48 8B 0D ?? ?? ?? ?? 48 85 C9 0F 84 CA 00 00 00 45 33 C0 BA 8F 00 00 00");
	ParamMan->GetMenuCommonParamEntry = (GetMenuCommonParamEntry*)getMenuCommonParamEntrySig.Scan(&EldenRingData, Align16);

	Signature getActionButtonParamEntrySig = Signature("40 57 48 83 EC 40 48 C7 44 24 20 FE FF FF FF 48 89 5C 24 50 "
		"48 89 6C 24 58 48 89 74 24 60 8B FA 48 8B F1 33 DB 85 D2 0F 88 21 01 00 00 48 8B 0D ?? ?? ?? ?? 48 85 C9 75 "
		"?? 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 4C 8B C8 4C 8D 05 ?? ?? ?? ?? BA B4 00 00 00 48 8D 0D ?? ?? ?? ?? E8 "
		"?? ?? ?? ?? 48 8B 0D ?? ?? ?? ?? 45 33 C0 41 8D 50 24");
	ParamMan->FindActionButtonParamEntry = (FindActionButtonParamEntry*)getActionButtonParamEntrySig.Scan(&EldenRingData, Align16);

	Signature enableBossBarSig = Signature("48 89 5C 24 08 48 89 74 24 10 57 48 83 EC 20 48 8B F9 41 8B F0 48 8B 0D "
		"?? ?? ?? ?? 8B DA 48 85 C9");
	...
	Signature disableBossBarSig = Signature("40 53 48 83 EC 20 8B D9 48 8B 0D ?? ?? ?? ?? 48 85 C9 75 ?? 48 8D 0D "
		"?? ?? ?? ?? E8 ?? ?? ?? ?? 4C 8B C8 4C 8D 05 ?? ?? ?? ?? BA B4 00 00 00 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? "
		"48 8B 0D ?? ?? ?? ?? 8B D3 E8");
	FeMan->_disableBossBarAddr = (uintptr_t)disableBossBarSig.Scan(&EldenRingData, Align16);

	Signature csFeManImp = Signature("48 8B 0D ?? ?? ?? ?? 8B DA 48 85 C9 75 ?? 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? "
		"4C 8B C8 4C 8D 05 ?? ?? ?? ?? BA B4 00 00 00 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8B 0D ?? ?? ?? ?? 8B D3 "
		"E8 ?? ?? ?? ?? 48 8B D8");
	...
	Signature handleDamage = Signature("48 8B C4 57 41 54 41 55 41 56 41 57 48 83 EC 60 48 C7 40 C8 FE FF FF FF 48 "
		"89 58 08");
	FeMan->_handleDmg = (uintptr_t)handleDamage.Scan(&EldenRingData, Align16);

	Signature applyEntityBarDmgSig = Signature("48 89 5C 24 08 48 89 6C 24 10 48 89 74 24 18 48 89 7C 24 20 41 56 "
		"48 83 EC 20 48 8B DA 45 33 F6 41 8B E9 41 8B F0 45 8B D6 44 8B C3 41 8B D6 48 8B F9 41 B9 FF FF FF FF");
	...
	Signature csSoundSig = Signature("48 8B 05 ?? ?? ?? ?? 48 85 C0 ?? ?? 83 CD 02 83 B8 ?? ?? ?? ?? 00 89 6C 24 "
		"?? 0F 95 C1");
	SoundIns = (CSSound**)csSoundSig.Scan(&EldenRingData, 0x3, 0x7);

	Signature executeActionButtonParamSig = Signature("48 89 5C 24 08 57 48 81 EC 90 00 00 00 48 8B 84 24 E0 00 00 "
		"00 41 0F B6 D9 48 8B 0D ?? ?? ?? ?? 8B FA 0F 29 B4 24 80 00 00 00");
	...
	Signature enemyInsDtor = Signature("48 89 5C 24 08 57 48 83 EC 20 8B DA 48 8B F9 E8 ?? ?? ?? ?? F6 C3 01 74 ?? "
		"BA D0 05 00 00");
	FeMan->_enemyInsDtor = (uintptr_t)enemyInsDtor.Scan(&EldenRingData, Align16);

	Signature feManCtor = Signature("48 89 4C 24 08 55 53 56 57 41 54 41 55 41 56 41 57 48 8B EC 48 83 EC 48 48 C7 "
		"45 E8 FE FF FF FF 48");
	FeMan->_feManCtor = (uintptr_t)feManCtor.Scan(&EldenRingData, Align16);

	Signature updateUIBarStructs = Signature("40 55 56 57 41 54 41 55 41 56 41 57 48 83 EC 60 48 C7 44 24 30 FE FF "
		"FF FF 48 89 9C 24 B0 00 00 00 48 8B 05 ?? ?? ?? ?? 48 33 C4 48 89 44 24 58 48");
	...
	return EventMan && EventMan->SetEventFlagAddress && DebugMan->DisableOpenMapInCombatLocation
		&& DebugMan->CloseMapInCombatLocation && DebugMan->DisableCrafingInCombatLocation && ParamMan->SoloParamRepository
	&& ParamMan->FindEquipParamWeaponFunc && ParamMan->FindEquipParamProtectorFunc && ParamMan->FindEquipParamGoodsFunc
	&& ParamMan->FindEquipMtrlSetParamFunc && ParamMan->GetMenuCommonParamEntry && ParamMan->FindActionButtonParamEntry
	...
	&& FeMan->_executeActionButtonParamProxy&& FeMan->_actionButtonParamImp && FeMan->_feManCtor && FeMan->_updateUIBarStructs;
}
